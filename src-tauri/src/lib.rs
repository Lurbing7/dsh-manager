...
/// Two tray icons: dim = harness not running, vivid = harness running.
/// Generated by tools/make_icons.py from the source artwork.
...
/// How often the tray polls the harness port.
...
fn tray_image(running: bool) -> tauri::image::Image<'static> {
    let bytes: &'static [u8] = if running {
        TRAY_RUNNING_PNG
    } else {
        TRAY_STOPPED_PNG
    };
...
fn refresh_tray(app: &AppHandle) {
    ...
    let _ = tray.set_icon(Some(tray_image(running)));
